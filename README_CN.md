...
## v1.8.3 新增：重要难点直接说给用户听

隐藏复杂度扫描不再只停留在 AI 内部。

当项目存在用户容易低估、可能导致返工、成本上升或架构变化的真实难点时，AI 会先用一句明显提醒开头：

> ⚠️ **这个很重要，请先看一下**

随后只用人话说 2～4 个关键难点，可以稍微狠一点、幽默一点，但不生成报告、不新增第四个文件、不重复刷屏。没有新证据时，同一批难点不会在多轮对话里反复提醒。
...
v1.8.3 另外针对“用户可见的重要难点提醒”新增了本地规则级压力测试：

- 6 项静态规则检查
- 16 个不同项目的单轮输出场景
- 6 组多轮对话，共 24 轮
- 5,000 组随机多轮状态变异，共 45,167 轮
- 提醒样例平均 114.5 个紧凑字符，最大 128 / 320
- 当前失败：0

这些结果验证的是规则完整性、输出长度、触发条件、多轮去重和抗规则偏移样例，不代表所有外部模型都能 100% 遵守。
